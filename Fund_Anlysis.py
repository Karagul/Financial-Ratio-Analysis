...
if __name__ == '__main__':
    ...
    ### Read the file
    df_data = r.concat_data('Fund Analysis.xlsx')
    ...
    fig, axes = plt.subplots(nrows=2, ncols=2)    
    rolling_corr_df.plot(ax=axes[0,0]); axes[0,0].set_title('36 Months Rolling Correlation')
    rolling_sharpe_ratio_df.plot(ax=axes[0,1]); axes[0,1].set_title('36 Months Rolling Sharpe Ratio')
    rolling_sortino_ratio_df.plot(ax=axes[1,0]); axes[1,0].set_title('36 Months Rolling Sortino')
    rolling_omega_ratio_df.plot(ax=axes[1,1]); axes[1,1].set_title('36 Months Rolling Omega Ratio')    
    
    
    dd_df[['TeamCo Client Composite', 'HFRI Fund Weighted Composite Index', 'Russell 3000']].plot(title='Draw Down - Fund vs Competitors vs Benchmark')
